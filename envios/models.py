...
# ======================================================
# 🔥 ENCOMIENDA (TU MODELO PRINCIPAL)
# ======================================================

class Encomienda(models.Model):

    # 👉 AQUÍ está bien aplicado tu QuerySet personalizado
        ...
        if self.fecha_entrega_est and self.fecha_entrega_real:
            if self.fecha_entrega_real < self.fecha_entrega_est:
                errors['fecha_entrega_real'] = ValidationError(
                    'La fecha real no puede ser antes de la estimada.'
                )
    ...
    def save(self, *args, **kwargs):
        ...
        # Generar fecha estimada
        if not self.fecha_entrega_est and self.ruta:
            self.fecha_entrega_est = timezone.now().date() + timedelta(days=self.ruta.dias_entrega)

        # Calcular costo
        if not self.costo_envio and self.ruta:
